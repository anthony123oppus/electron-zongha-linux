...
function App() {
  ...
  const handleClickPost = async () => {
    try {
      const response = await window.electron.postApiRequest<
        TypePost,
        PostResponse
      >({
  ...
  const putRequest = async () => {
    try {
      const response = await window.electron.putApiReqeust<
        TypePost,
        PutResponse
      >({
  ...
  const getRequest = async () => {
    try {
      const response = await window.electron.getApiRequest<CatFactReqTypes>({
        url: "fact",
      });
  ...
  const deleteRequest = async () => {
    try {
      const response = await window.electron.deleteApiRequest<{
        message: string;
      }>({
  ...
  const patchRequest = async () => {
    try {
      const response = await window.electron.patchApiRequest<
        { name: string },
        PutResponse
      >({
  ...
  return (
    <Fragment>
      ...
      <div>{putSample && putSample.name}</div>
      <div>{deleteMessage}</div>
      <div>{patchSample && patchSample.name}</div>

      <div onClick={handleClickPost}>hello try post</div>
      <div onClick={getRequest}>Smaple Gte request</div>

      <div onClick={putRequest}>sample put request</div>

      <div onClick={deleteRequest}>sample delete request</div>

      <div onClick={patchRequest}>sample patch request</div>
    </Fragment>
  );
}
